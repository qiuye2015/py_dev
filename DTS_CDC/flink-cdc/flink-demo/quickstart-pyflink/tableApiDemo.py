...
创建 TableEnvironment
创建源表和结果表
创建一个作业：该作业读取表 source 中的数据，进行一些变换，然后将结果写入表 sink
启动 Flink Python Table API 作业

Table API 提供了一种机制来查看 Table 的逻辑查询计划和优化后的查询计划。 这是通过 Table.explain() 或者 StatementSet.explain() 方法来完成的
"""
import argparse
import logging
import sys
...
def word_count(input_path, output_path):
    ...
    if output_path is not None:
        t_env.create_temporary_table(
            'sink',
            TableDescriptor.for_connector('filesystem')
            .schema(Schema.new_builder().column('word', DataTypes.STRING())
        ...
        t_env.create_temporary_table(
            ...
            .schema(Schema.new_builder().column('word', DataTypes.STRING()).column('count', DataTypes.BIGINT())
            .build()
            )
            .build()
        )
