...
search_condition = 'co21sf1bp100ep250/'  # 用途=普通住宅
subway_dict = {
    'li647': '1号线',
    'li648': '2号线',
    'li656': '4号线大兴线',
...
def get_house_detail_by_api(house_url):
    ...
    if response.status_code == 200:
        ...
        house_info.update(
            dict(
                pure_shoufu=pure_shoufu,
                total_shoufu=total_shoufu,
                total_price=total_price,
                evaluation=evaluation,
                evaluation_rate=evaluation_rate,
                month_pay_debx=month_pay_debx,
                month_pay_debj=month_pay_debj,
                tax_total=tax_total,
                cost_jingjiren=cost_jingjiren,
            )
        )
...
def get_house_detail_by_html(house_url):
    ...
    return dict(
        pure_shoufu=pure_shoufu,
        total_shoufu=total_shoufu / 10000,
...
def main(args):
    ...
    with open('history/' + file_name + '.txt', 'w') as f:
        ...
        for subway in line_order:
            ...
            print(f"subway_house_list len={len(subway_house_list)}")
            for house_url in subway_house_list:
                ...
                out_put = (
                    ...
                    + subway_dict[subway]
                    + " 总首付："
                    + str(int(house_info.get('total_shoufu', 0)))
                    + "（万）\n        网页获取："
                    + str(house_info)
                    + "\n        api获取："
                    + str(house_info2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--shoufu', help='最大首付，单位：万元', default=280, type=int)
    parser.add_argument('-a', '--area', help='最小面积，单位：平方米', default=50, type=int)
    parser.add_argument('-g', '--age', help='最大房龄，单位：年', default=20, type=int)
