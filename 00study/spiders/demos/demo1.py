    ...
    def crawl_spider(self):
        while True:
            if self.queue.empty():  # 如果队列为空，则跳出
                ...
                url = 'https://www.qiushibaike.com/Shr/page/{}/'.format(
                    str(page))
                headers = {
...
class Parser_thread(threading.Thread):
    ...
    def parse_data(self, item):
        ...
        try:
            ...
            result = html.xpath(
                '//div[contains(@id,"qiushi_tag")]')  # 匹配所有段子内容
            for site in result:
                try:
                    img_url = site.xpath('.//img/@src')[0]  # 糗事图片
                    title = site.xpath('.//h2')[0].text  # 糗事题目
                    content = site.xpath(
                        './/div[@class="content"]/span')[0].text.strip()  # 糗事内容
                    response = {
                        'img_url': img_url,
                        'title': title,
                        'content': content
                    }  # 构造json
                    json.dump(response, fp=self.file,
